			...
			<div class="panel panel-default">
				...
				<div class="panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover"
						...
						<thead>
							<tr>
								<th style="width: 75px;"><?=lang('news.field_news_id')?></th>
								<th><?=lang('news.field_news_title')?></th>
								<th><?=lang('news.field_news_type')?></th>
...
$(document).ready(function() {
	...
	var table = $('#dataTables-example').DataTable( {
	    ...
	    "bSort":false,
	    "ajax": {
	    	      url: "<?=site_url('News/ListProcessing')?>",
            	type: "POST",
            	pages: 5,
							ajax: "data.json"
		    },
	    "columns": [
	        {  name: "id",data: "id" },
	        {  name: "news_title",data: "news_title" },
	...
	$('#dataTables-example tbody').on( 'click', 'button', function () {
        ...
        if($(this).val() == 'update')  {
        		...
        		var frameSrc = "<?php echo base_url('index.php/');?>"+"/News/UpdateForm/"+rowData['id'];
	...
	var rowData = $('#dataTables-example').DataTable().row($tr).data();
	        ...
	        if($(this).val() == 'delete')  {
	        		...
	        		var frameSrc = "<?php echo base_url('index.php/');?>"+"/News/deleteForm/"+rowData['id'];
	        			$('iframe').attr("src",frameSrc);
