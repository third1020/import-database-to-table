<link href="<?=base_url('assets')?>/vendor/datatables-plugins/dataTables.bootstrap.css"
	rel="stylesheet">
<form action="<?php echo base_url();?>index.php/Problems/delete" method="post"></form>
<form action="<?php echo base_url();?>index.php/Problems/update" method="post"></form>
...
<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header"><?=lang('problems.pageHeader')?></h3>
		<!-- 		<div class=""> -->
		<div class="panel-body">
			<form action="<?php echo base_url('index.php/Problems/Add');?>"  method="post">
				<button type="submit" class="btn btn-success">
					<i class="glyphicon glyphicon-plus"></i> <?=lang('problems.bt_add_problems')?>
				</button>
			</form>

			<div class="panel panel-default">
				<div class="panel-heading"><?=lang('problems.pageHeaderTable') ?></div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover"
						...
						<thead>
							<tr>
								<th><?=lang('problems.id')?></th>
								<th><?=lang('problems.problems_tital')?></th>
								<th><?=lang('problems.problems_status')?></th>
...
$(document).ready(function() {
	...
	var table = $('#dataTables-example').DataTable( {
	    ...
	    "bSort":false,
	    "ajax": {
	    	      url: "<?=site_url('Problems/dataProcessing')?>",
            	type: "POST",
            	pages: 5,
							ajax: "data.json"
		    },
	    "columns": [
	        {  name: "id",data: "id" },
	        {  name: "problems_tital",data: "problems_tital" },
	        {  name: "problems_status",data: "problems_status" },
 ...
 // }, 1000 );
	...
	$('#dataTables-example tbody').on( 'click', 'button', function () {
        ...
        if($(this).val() == 'update')  {
        		...
        		var frameSrc = "<?php echo base_url('index.php/Problems/updateForm/');?>"+rowData['id'];
...
// 		console.log($(this).val());
	...
	var rowData = $('#dataTables-example').DataTable().row($tr).data();
	if($(this).val() == 'delete')  {
		var frameSrc = "<?php echo base_url('index.php/Problems/deleteForm/');?>"+rowData['id'];
