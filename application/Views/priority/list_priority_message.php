<link href="<?=base_url('assets')?>/vendor/datatables-plugins/dataTables.bootstrap.css"
	rel="stylesheet">
<form action="<?php echo base_url();?>index.php/priority/delete" method="post"></form>
<form action="<?php echo base_url();?>index.php/priority/update" method="post"></form>
...
<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header"><?=lang('priority.pageHeader')?></h3>
		<!-- 		<div class=""> -->
		<div class="panel-body">


			<div class="panel panel-default">
				<div class="panel-heading"><?=lang('priority.pageHeaderTable') ?></div>
				<!-- /.panel-heading -->
				<div class="panel-body">
					<table width="100%" class="table table-striped table-bordered table-hover"
						...
						<thead>
							<tr>
								<th><?=lang('priority.id')?></th>
								<th><?=lang('priority.priority_name')?></th>
								<th><?=lang('priority.priority_status')?></th>
...
$(document).ready(function() {
	...
	var table = $('#dataTables-example').DataTable( {
	    ...
	    "bSort":false,
	    "ajax": {
	    	      url: "<?=site_url('priority/dataProcessing')?>",
            	type: "POST",
            	pages: 5,
							ajax: "data.json"
		    },
	    "columns": [
	        {  name: "id",data: "id" },
	        {  name: "priority_name",data: "priority_name" },
	        {  name: "priority_status",data: "priority_status" },
 ...
 // }, 1000 );
	...
	$('#dataTables-example tbody').on( 'click', 'button', function () {
        ...
        if($(this).val() == 'update')  {
        		...
        		var frameSrc = "<?php echo base_url('index.php/priority/updateForm/');?>"+rowData['id'];
...
// 		console.log($(this).val());
	...
	var rowData = $('#dataTables-example').DataTable().row($tr).data();
	if($(this).val() == 'delete')  {
		var frameSrc = "<?php echo base_url('index.php/priority/deleteForm/');?>"+rowData['id'];
