...
<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header"><?=lang('user.pageHeader')?></h3>
		<!-- 		<div class=""> -->

		<div class="panel-body">
			<div class="form-group">
				<a href="<?=site_url('/User/Add')?>" class="btn btn-success">
					<i class="glyphicon glyphicon-plus"></i> <?=lang('user.bt_add_user')?>
				</a>
			</div>
			<div class="panel panel-default">
				<div class="panel-heading"><?=lang('user.pageHeaderTable') ?></div>
				<!-- /.panel-heading -->
				<div class="panel-body">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				<button type="button" id="btn-inside" cla ss="btn btn-default" data-dismiss="modal">Close</button>
			</div>
		</div>
	</div>
</div>
...
$(document).ready(function() {
	...
	var table = $('#dataTables-example').DataTable( {
	    ...
	    "bSort":false,
	    "ajax": {
	    	      url: "<?=site_url('User/dataProcessing')?>",
            	type: "POST",
            	pages: 5,
							ajax: "data.json"
		    },
	...
	$('#dataTables-example tbody').on( 'click', 'button', function () {
        ...
        if($(this).val() == 'update')  {
        		...
        		var frameSrc = "<?php echo base_url('index.php/');?>"+"/User/UpdateForm/"+rowData['id'];
	...
	var rowData = $('#dataTables-example').DataTable().row($tr).data();
	        ...
	        if($(this).val() == 'delete')  {
	        		...
	        		var frameSrc = "<?php echo base_url('index.php/');?>"+"/User/deleteForm/"+rowData['id'];
	        			$('iframe').attr("src",frameSrc);
