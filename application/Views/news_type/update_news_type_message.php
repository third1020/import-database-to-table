...
<?php } ?>
<?php echo form_open_multipart('NewsType/updateForm/'. $newsType->id); ?>
							...
							<div class="col-lg-6">
								<div class="form-group">
									<label><?=lang('news.nesw_type_name')?></label> <input name="type_name"
										class="form-control" placeholder="Enter text" value="<?=isset($newsType->type_name) ? $newsType->type_name  : ''?>">
								</div>
								<!-- /.col-lg-6 (nested) -->
							</div>
							<!-- /.row (nested) -->
				...
				<input type="hidden" name="news_type_id" value="<?=isset($newsType->id) ? $newsType->id  : ''?>">
                         <?php form_close(); ?>
